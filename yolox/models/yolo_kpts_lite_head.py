...
class YOLOXHeadKPTSLite(nn.Module):
    def __init__(
        self,
        # num_classes,
        width=1.0,
    ...
    def forward(self, xin):#, labels=None, imgs=None):
        ...
        for k, (kpts_conv, x) in enumerate(
            zip(self.kpts_convs, xin)
        ):
            x = self.stems[k](x)
            # cls_x = x
            # reg_x = x
            ...
            # if self.training:
            #     output = torch.cat([kpts_output], 1)
            ...
            #         kpts_output = kpts_output.view(
            #             batch_size, self.n_anchors, 3*self.num_kpts, hsize, wsize
            #         )
    ...
    def get_output_and_grid(self, output, k, stride, dtype):
        ...
        output = (2*output -0.5 +  kpt_grids.repeat(1,1,self.num_kpts)) * stride
        return output, grid
    ...
    def decode_outputs(self, outputs, dtype):
        ...
        outputs = (2 * outputs - 0.5  + kpt_grids_repeat) * strides
    ...
    #             l1_target_kpts = outputs.new_zeros((0, 2*self.num_kpts))
    ...
    #                 )
    #                 l1_target_kpts = self.get_l1_target_kpts(
    #                     outputs.new_zeros((num_fg_img, 2*self.num_kpts)),
    #                     gt_kpts_per_image[matched_gt_inds],
    #                     expanded_strides[0][fg_mask],
    #                     x_shifts=x_shifts[0][fg_mask],
    #                     y_shifts=y_shifts[0][fg_mask],
    #                 )
    ...
    #             l1_targets_kpts.append(l1_target_kpts)
